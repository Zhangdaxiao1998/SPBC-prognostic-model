...
import pandas as pd
from SPBC_progn_model_code import predict_survival  # 假设你在rsf_newpred.py中定义了predict_survival函数

# 定义一个函数用于预测和展示结果

def predict_patient_survival(new_patient, target_time):
    # 调用模型进行预测
    survival_prob_at_t, death_risk_at_t, upper_ci_at_t, lower_ci_at_t = predict_survival(new_patient, target_time)
    # 显示预测结果
...
if st.button('输出模型预测结果'):
    predict_patient_survival(new_patient, target_time)